...
public class TesseractWrapper
{
    ...
    public string Recognize(RenderTexture texture, bool isDraw)
    {
        RenderTexture.active = texture;
        if (_highlightedTexture == null || _highlightedTexture.width != texture.width || _highlightedTexture.height != texture.height)
            _highlightedTexture = new Texture2D(texture.width, texture.height, TextureFormat.RGB24, false);
        _highlightedTexture.ReadPixels(new Rect(0, 0, texture.width, texture.height), 0,  0);
        _highlightedTexture.Apply();
        RenderTexture.active = null;
        return Recognize(_highlightedTexture, isDraw);
    }
    
    public string Recognize(Texture2D texture, bool isDraw)
    {
        if (_tessHandle.Equals(IntPtr.Zero))
            return null;
        ...
        _maxRect = new Rect();
        float maxRectArea = float.MinValue;
        for (int index = 0; index < boxes.Length; index++)
        {
            if (confidence[index] >= MinimumConfidence)
            {
                ...
                var y = _highlightedTexture.height - box.y - box.h;
                var boundingRect = new Rect(box.x, y, box.w, box.h);
                var area = boundingRect.width * boundingRect.height;
                if (area > maxRectArea)
                {
                    maxRectArea = area;
                    _maxRect = boundingRect;
                }
                
                if (isDraw)
                {
                    DrawLines(_highlightedTexture, boundingRect, Color.green);
                }
            }
        }
    ...
    public Texture2D CollectMaxRect()
    {
        var x = (int)_maxRect.x;
        var y = (int)_maxRect.y;
        var w = (int)(_maxRect.width - float.Epsilon) + 1;
        var h = (int)(_maxRect.height - float.Epsilon) + 1;

        if (_highlightedTexture == null || x + w >= _highlightedTexture.width || y + h >= _highlightedTexture.height)
            return null;
        var texture = new Texture2D(w, h);
        texture.SetPixels(_highlightedTexture.GetPixels(x, y, w, h));
        texture.Apply();
        return texture;
    }
